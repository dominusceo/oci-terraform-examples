...
/*
resource "oci_sch_service_connector" "audit_events_connector" {
  compartment_id = var.compartment_ocid
  display_name   = "audit_events_connector"
  source {
    kind = "logging"
    log_sources {
      compartment_id = var.compartment_ocid
      //log_group_id   = oci_logging_log_group._Audit_Include_Subcompartment.id
      //log_id         = oci_logging_log.vcn_flow_logs.id
    }
  }
  target {
    kind      = "streaming"
    stream_id = oci_streaming_stream.splunk_audit_events_stream.id
  }

  defined_tags = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}

resource "oci_sch_service_connector" "audit_events_connector_ob" {
  compartment_id = var.compartment_ocid
  display_name   = "audit_events_connector_object_storage"
  source {
    kind = "logging"
    log_sources {
      compartment_id = var.compartment_ocid
