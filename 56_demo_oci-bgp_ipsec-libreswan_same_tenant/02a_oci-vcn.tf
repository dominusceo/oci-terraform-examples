...
##################################################################################
# Create private subnet for OCI VCN
##################################################################################
resource "oci_core_subnet" "oci-vcn-priv-subnet" {
  cidr_block                 = var.oci_priv_subnet_cidr
  display_name               = "oci-vcn-priv-subnet"
  dns_label                  = "ocivcnprivsub"
  compartment_id             = var.compartment_ocid
  vcn_id                     = oci_core_vcn.oci-vcn.id
  security_list_ids          = [oci_core_vcn.oci-vcn.default_security_list_id]
  prohibit_public_ip_on_vnic = true
  route_table_id = oci_core_route_table.oci-default-priv-route-table.id
}
...
##################################################################################
# Create NATGW for OCI VCN
##################################################################################
resource "oci_core_nat_gateway" "oci-nat-gateway" {
  #Required
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_vcn.oci-vcn.id
  #Optional
  display_name   = "oci-natgw"
 }

 // Create NAT 
resource "oci_core_route_table" "oci-default-priv-route-table" {
  #Required
  compartment_id             = var.compartment_ocid
  vcn_id         = oci_core_vcn.oci-vcn.id
  display_name = "Default Private Route Table"

  route_rules {
    #Required
    network_entity_id = oci_core_nat_gateway.oci-nat-gateway.id
    #Optional
    destination = "0.0.0.0/0"
  }
}