...
resource null_resource lb-certificates-generate {
  provisioner local-exec {
    command = "sh ./certs/create-certificates.sh"
  }
}
...
resource "oci_load_balancer" "lb1" {
  ...
  compartment_id = var.compartment_ocid
  subnet_ids = [
    oci_core_subnet.pub_subnet1.id,
  ]
...
resource "oci_load_balancer_hostname" "test_hostname1" {
  #Required
  hostname         = "${oci_core_instance.webserver_node[0].display_name}.example.com"
  name             = oci_core_instance.webserver_node[0].display_name
  load_balancer_id = oci_load_balancer.lb1.id
}

resource "oci_load_balancer_hostname" "test_hostname2" {
  #Required
  hostname         = "${oci_core_instance.webserver_node[1].display_name}.example.com"
  name             = oci_core_instance.webserver_node[1].display_name
  load_balancer_id = oci_load_balancer.lb1.id
}
...
resource "oci_load_balancer_backend" "lb-backend1" {
  ...
  ip_address       = oci_core_instance.webserver_node[count.index].private_ip
  port             = 443
  backup           = false
  drain            = false
  offline          = false
...
output "lb_public_ip-and_nodes" {
  value = [oci_load_balancer.lb1.ip_address_details, oci_core_instance.webserver_node[0].private_ip, oci_core_instance.webserver_node[1].private_ip]
}